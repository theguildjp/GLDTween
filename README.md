...
### 背景
iOSでは手触りやアニメーションが重要視されながら、実際のプログラミング環境ではアニメーションに対する資産が不足していました。UIAnimationは機能が貧弱であり、CoreAnimationは強力であるものの記述が複雑で、表現レベルのプログラミングを直感的に書くにはほど遠い環境です。Facebookがリリースした[POP](https://github.com/facebook/pop)はこれらへの解の１つですが、最低限の機能しか搭載しておらず、やはり高度なアニメーションには大量のカスタムクラスを書く必要がありました。
